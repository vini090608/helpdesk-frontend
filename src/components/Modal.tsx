...
export default function Modal({ isOpen, onClose }: ModalProps) {
  ...
  return (
    <>
      ...
      {/* Modal */}
      <div className="absolute bottom-4 left-2/11 w-50 bg-gray-100 rounded-2xl p-3 z-40" onClick={(e) => e.stopPropagation()}  >
        <span className="text-gray-400 text-xxs p-3">OPÇÕES</span>

        <button onClick={() => setProfileOpen(true)} className="p-2 flex gap-2 bg-transparent" >
          <img src={profile} alt="" />
          <span className="text-gray-500 text-heading-md">Perfil</span>
        </button>

        <button onClick={() => { user.remove(); navigate("/"); }} className="p-2 flex gap-2 bg-transparent" >
          <img src={logOut} alt="" />
